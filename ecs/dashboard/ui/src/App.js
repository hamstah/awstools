...
const Check = (props) => <FontAwesome icon="check-circle" {...props} />;

const Pause = (props) => <FontAwesome icon="pause-circle" {...props} />;
  ...
  processData = (data) => {
    ...
    for (const accountState of data["state"]) {
      ...
      for (const service of accountState["services"]) {
        ...
        services[clusterName][name][accountState["account"]["account_name"]] =
          service;
      }
    }
    this.setState({
              ...
              {messages.map((message, index) => (
                <div
                  className={[
                    "notification",
                    "is-small",
                    message["type"] == "error" ? "is-danger" : "is-primary",
                  ].join(" ")}
                  key={"message-" + index}
                >
                  {message.message}
              ...
              {Object.keys(services)
                ...
                .map((clusterName) => (
                  <tbody key={"body-" + clusterName}>
                    ...
                    {Object.keys(services[clusterName])
                      .sort()
                        ...
                        <tr key={clusterName + "-" + serviceName + "-row"}>
                          ...
                          {accounts.map((account, index) => {
                            ...
                            if (s) {
                              if (s["status"] == "ACTIVE") {
                                if (s["desired_count"] == s["running_count"]) {
                                  component["count"] = <Check color="green" />;
                                } else {
                                  component["count"] = (
                                    <FontAwesome
                                      color="red"
                              ...
                              for (var accountState of data["state"]) {
                                if (
                                  accountState["account"]["account_name"] ===
                                  account["account_name"]
                                ) {
                              ...
                              if (
                                event["message"].indexOf("steady state") != -1
                              ) {
                                component["event"] = <Check color="green" />;
                              } else {
                                component["event"] = (
                                  <FontAwesome
                                    color="red"
                            ...
                            return subheaders.map((subhead, colIndex) => (
                              <td
                                className={[
                                  vBorderStyle(colIndex, subheaders),
                                  hBorderStyle(
                                    rowIndex,
                                    Object.keys(services[clusterName])
                                  ),
                                ...
                                style={{
                                  textAlign: "center",
                                  backgroundColor: component[subhead]
                                    ? "transparent"
                                    : "#eee",
                                }}
                                key={
